      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex justify-between items-center">
          <div>
            ...
            <p className="mt-2 text-gray-600">{canteen.name} - {canteen.location}</p>
          </div>
          <button
            onClick={() => refetch()}
              ...
              <div className="bg-white rounded-lg shadow p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-gray-900 mt-2">{analytics?.completedOrders || 0}</p>
                  </div>
              ...
              <div className="bg-white rounded-lg shadow p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">{analytics?.totalReviews || 0} reviews</p>
                  </div>
                ...
                {analytics?.popularDishes && analytics.popularDishes.length > 0 ? (
                  <div className="space-y-3">
                    {analytics.popularDishes.slice(0, 5).map((dish, index) => (
                      <div key={dish._id} className="flex items-center justify-between py-2 border-b last:border-b-0">
                        <div className="flex items-center gap-3">
                          <span className="text-lg font-bold text-gray-400">#{index + 1}</span>
                          <div>
                            <p className="font-medium text-gray-900">{dish.dishInfo[0]?.name || 'Unknown'}</p>
                            <p className="text-xs text-gray-500">{dish.count} orders</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="font-semibold text-gray-900">₹{dish.revenue}</p>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <button
                  ...
                  className="flex items-center justify-center gap-3 bg-indigo-600 text-white px-6 py-4 rounded-lg hover:bg-indigo-700 transition-colors font-medium"
                >
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-3 bg-green-600 text-white px-6 py-4 rounded-lg hover:bg-green-700 transition-colors font-medium"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button
                  onClick={() => router.push('/manage/canteen')}
                  className="flex items-center justify-center gap-3 bg-purple-600 text-white px-6 py-4 rounded-lg hover:bg-purple-700 transition-colors font-medium"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
